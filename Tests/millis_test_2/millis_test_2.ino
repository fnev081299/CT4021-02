...
unsigned long delayTime = 10000; //20 seconds
...
void loop() {
  ...
  { 
    ...
    delayStart = millis();
    delayRunning = true;
    if (light = true && delayRunning && ((millis() - delayStart) <= delayTime)) // if the light is on
    {
    ...
    if (card = false && delayRunning && ((millis() - delayStart) >= delayTime)){
        setColor(255, 0, 0);
          tone(speaker, 300, 250);
          Serial.println(" Access Denied");
          delay(2000);
